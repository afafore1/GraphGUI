...
public class Algorithms {
    ...
    public static  void Bfs(Vertex source) {
        ...
        Model.graph.printlnConsole("Order is " + conn);
    }
    ...
    public static  void execute(Vertex source) {
        ...
        vertex = GraphifyGUI.getNode();
        edges = GraphifyGUI.getEdges();
        sNodes = new HashSet<>(); // settled Model.vertices will be placed in this setShortestPath
        uSNodes = new HashSet<>(); // unsettled Model.vertices will be placed in this setShortestPath
        dist = new HashMap<>(); // weight to get to node
        reset();
        dist.put(source, 0); // first setShortestPath source to 0
        uSNodes.add(source); // add source to unsettled Model.vertices
        while (uSNodes.size() > 0) { // do this until no more unsettled Model.vertices
            Vertex v = getMin(uSNodes); // we use min node from unsettled Model.vertices each time to process
            sNodes.add(v); // add it to settled Model.vertices
    ...
    public static ArrayList BfsSuModel.graphest(Vertex source, int num) {
        vertex = GraphifyGUI.getNode();
        reset();
        q = new LinkedList<>(); // FIFO
        source.wasVisited = true; // marked as Model.visited
